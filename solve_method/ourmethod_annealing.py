from box import Box
from solve_method import ourmethod
...
def solve(container, boxes_list, time_limit):
  ...
  loaded_rate = get_scores(container, ourmethod.solve(container, boxes_list, time_limit))
  ...
  for i in range(1, 3):
    ...
    while t >= end_temperature and time.time() - start_time <= time_limit / 1000:
      for _ in range(L_t):
        ...
        f_neighor = get_scores(container, ourmethod.solve(container, boxes_list_neighbor, time_limit))
        delta_f = f_neighor - f
        if delta_f > 0:
          f = f_neighor
      ...
      t *= drop_rate
      drop_rate *= 0.8
  return ourmethod.solve(container, boxes_list_best, time_limit)